...
import { SEED_NEWS, SEED_RANKINGS } from "../data/seed.js";
...
export function RankingsNews() {
  ...
  const safeNews = Array.isArray(news) && news.length > 0 ? news : SEED_NEWS;
  const safeRankings = Array.isArray(rankings) && rankings.length > 0 ? rankings : SEED_RANKINGS;
  const lead = safeNews[0];
  return (
      ...
      <div className="bc-grid-rn" style={{ maxWidth: 1320, margin: "0 auto", display: "grid", gridTemplateColumns: "1.05fr 1fr", gap: 48, padding: "34px 0 40px" }}>
        <div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {safeRankings.slice(0, 6).map((r, i) => {
        ...
        <div>
          ...
          <Link to={lead.slug ? `/news/${lead.slug}` : "/news"} className="bc-card" style={{ borderRadius: 12, overflow: "hidden", border: "1px solid var(--bc-line)", marginBottom: 14, cursor: "pointer", display: "block", textDecoration: "none", color: "inherit" }}>
            <BcPhoto label="LEAD STORY" style={{ height: 156 }} />
            <div style={{ padding: "14px 18px", background: "var(--bc-panel)" }}>
              <div style={{ fontFamily: "var(--bc-sans)", fontWeight: 700, fontSize: 11, letterSpacing: "0.12em", color: "var(--bc-accent)", marginBottom: 6 }}>{lead.kicker.toUpperCase()}</div>
              <div style={{ fontFamily: "var(--bc-sans)", fontWeight: 700, fontSize: 20, lineHeight: 1.15, textWrap: "pretty" }}>{lead.title}</div>
              <div style={{ fontFamily: "var(--bc-body)", fontSize: 12, color: "var(--bc-sub)", marginTop: 8 }}>{lead.time} · {lead.read} read</div>
            </div>
          </Link>
          {safeNews.slice(1).map((n, i) => (
